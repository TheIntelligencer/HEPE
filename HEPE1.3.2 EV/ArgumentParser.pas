...
type
  ...
  //ZEROGRAVIRY FIX: case insensitive TParseResultClass
  ZgParseResultCIS = class
  private
    FStoredValues: TStringListContainer;
    FStoredBools: TStringList;
    FUnnamedValues: TStringList;
  ...
  public
    ...
    function GetArgumentByStr(const Option: String): TArgument;
    function GetArgument(Index: Integer): TArgument;
  ...
  //ZEROGRAVITY FIX - case insensitive TArgumentParser
  ZgArgumentParserCIS = class
  private
    FDescription: String;
    FArguments: TObjectList;
    function GetArgumentsCount: Integer;
  public
    constructor Create(const Description: String = '');
    destructor Destroy; override;
    function ParseArgs: ZgParseResultCIS; overload;
    function ParseArgs(const TargetArgs: TStringList): ZgParseResultCIS; overload;
    procedure AddArgument(const Argument: TArgument); overload;
    procedure AddArgument(const Option, Dest: String;
        StoreAction: TStoreAction = saBool); overload;
    procedure AddArgument(const Option: String;
...
begin
  ...
  begin
    ...
    if (LeftStr(CurrentParam, 1) = '-') or (LeftStr(CurrentParam, 1) = '/') then
    begin
...
function TArgumentParser.GetArgumentByStr(const Option: String): TArgument;
var
  I: Integer;
  Argument: TArgument;
begin
...
(* ZgArgumentParserCIS *)
constructor ZgArgumentParserCIS.Create(const Description: String = '');
begin
  FDescription := Description;
  FArguments := TObjectList.Create;
end;

destructor ZgArgumentParserCIS.Destroy;
begin
  FArguments.Free;
  inherited Destroy;
end;

function ZgArgumentParserCIS.ParseArgs: ZgParseResultCIS;
var
  Params: TStringList;
begin
  Params := GetParamStrAsList(False);
...
function ZgArgumentParserCIS.ParseArgs(const TargetArgs: TStringList): ZgParseResultCIS;
var
  CurrentIndex: Integer;
  CurrentParam: String;
  SeparatorPosition: Integer;
  ...
  Argument: TArgument;
begin
  Result := ZgParseResultCIS.Create;
  CurrentIndex := 0;
  while CurrentIndex < TargetArgs.Count do
  begin
    CurrentParam := LowerCase(TargetArgs[CurrentIndex]);
    if (LeftStr(CurrentParam, 1) = '-') or (LeftStr(CurrentParam, 1) = '/') then
    begin
      SeparatorPosition := Pos('=', CurrentParam);
      if SeparatorPosition <> 0 then
...
procedure ZgArgumentParserCIS.AddArgument(const Argument: TArgument);
begin
  FArguments.Add(Argument);
end;

procedure ZgArgumentParserCIS.AddArgument(const Option, Dest: String;
    StoreAction: TStoreAction = saBool);
var
  Argument: TArgument;
begin
  Argument := TArgument.Create(LowerCase(Option), LowerCase(Dest), StoreAction);
  AddArgument(Argument);
end;

procedure ZgArgumentParserCIS.AddArgument(const Option: String;
    StoreAction: TStoreAction = saBool);
var
  Dest: String;
begin
...
function ZgArgumentParserCIS.HasArgument(const Option: String;
    StoreAction: TStoreAction = saBool): Boolean;
var
  I: Integer;
  Argument: TArgument;
...
function ZgArgumentParserCIS.GetArgumentByStr(const Option: String): TArgument;
var
  I: Integer;
  Argument: TArgument;
begin
...
function ZgArgumentParserCIS.GetArgumentsCount: Integer;
begin
  Result := FArguments.Count;
end;

function ZgArgumentParserCIS.GetArgument(Index: Integer): TArgument;
begin
  Result := TArgument(FArguments[Index]);
end;

(* End of ZgArgumentParserCIS *)

(* ZgParseResultCIS *)
constructor ZgParseResultCIS.Create;
begin
  FStoredValues := TStringListContainer.Create;
  FStoredBools := TStringList.Create;
  FUnnamedValues := TStringList.Create;
...
destructor ZgParseResultCIS.Destroy;
begin
  FStoredValues.Free;
  FStoredBools.Free;
  FUnnamedValues.Free;
...
function ZgParseResultCIS.HasArgument(const Dest: String): Boolean;
begin
  Result := (FStoredBools.IndexOf(LowerCase(Dest)) <> -1) or FStoredValues.ContainsKey(LowerCase(Dest));
end;

function ZgParseResultCIS.GetValue(const Dest: String): String;
begin
  if HasArgument(Dest) then
    Result := FStoredValues[Dest]
  else
...
procedure ZgParseResultCIS.StoreBool(const Dest: String);
begin
  if not HasArgument(Dest) then
    FStoredBools.Add(Dest);
end;

procedure ZgParseResultCIS.StoreValue(const Dest: String; const Value: String);
begin
  if not HasArgument(Dest) then
    FStoredValues.Add(Dest, Value);
end;
(* End of ZgParseResultCIS *)
